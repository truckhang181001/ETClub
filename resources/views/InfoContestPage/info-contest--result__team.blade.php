<h1>KẾT QUẢ CUỘC THI</h1>
<div class="container-fluid ">
    <div class="row">
        <div class="col-4 silver upper text-center">
            <img class="avatar avatar-1" src="/img/InfoContestPage/Group 1916.png" alt="">
            <div>
                <h2 class="name name-1">Tên đội thi
                    <i class="fas fa-caret-down arrow-1"></i>
                    <div class="name-info name-info-1">
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                    </div>
                </h2>
            </div>
        </div>
        <div class="col-4 gold upper text-center">
            <img class="avatar avatar-2" src="/img/InfoContestPage/Group 1916.png" alt="">
            <div>
                <h2 class="name name-2">Tên đội thi
                    <i class="fas fa-caret-down arrow-2"></i>
                    <div class="name-info name-info-2">
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                    </div>
                </h2>
            </div>
        </div>
        <div class="col-4 copper upper text-center">
            <img class="avatar avatar-3" src="/img/InfoContestPage/Group 1916.png" alt="">
            <div>
                <h2 class="name name-3">Tên đội thi
                    <i class="fas fa-caret-down arrow-3"></i>
                    <div class="name-info name-info-3">
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                    </div>
                </h2>
            </div>
        </div>
    </div>
    <div class="row">
        <img class="center" src="/img/InfoContestPage/Trophie.png" alt=""> 
    </div>
    <div class="row onsolationjustify-content-around">
        <div class="col-6 consolation-1 below text-center">
            <div class="h2-style--yellow">KHUYẾN KHÍCH</div>
            <img class="avatar avatar-4" src="/img/InfoContestPage/Group 1916.png" alt="">
            <div>
                <h2 class="name name-4">Tên đội thi
                    <i class="fas fa-caret-down arrow-4"></i>
                    <div class="name-info name-info-4">
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                    </div>
                </h2>
            </div>
        </div>
        <div class="col-6 consolation-2 below text-center">
            <div class="h2-style--yellow">KHUYẾN KHÍCH</div>
            <img class="avatar avatar-5" src="/img/InfoContestPage/Group 1916.png" alt="">
            <div>
                <h2 class="name name-5">Tên đội thi
                    <i class="fas fa-caret-down arrow-5"></i>
                    <div class="name-info name-info-5">
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                    </div>
                </h2>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-12 beloved text-center">
            <div class="h2-style--yellow">THÍ SINH ĐƯỢC YÊU THÍCH NHẤT</div>
            <img class="avatar avatar-6" src="/img/InfoContestPage/Group 1916.png" alt="">
            <div>
                <h2 class="name name-6">Tên đội thi
                    <i class="fas fa-caret-down arrow-6"></i>
                    <div class="name-info name-info-6">
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                        <div class="name-info_detail d-flex">
                            <div class="member-name">Nguyễn Văn A</div>
                            <div class="d-flex info">
                                <div class="member-majors">Hệ thống thông tin</div>
                                <span class="member-hyphen"> - </span>
                                <div class="member-year-school">K46 - UEH</div>
                            </div>
                        </div>
                    </div>
                </h2>
            </div>
        </div>
    </div>
    <div class="sponsor-blur">
        <div class="row text-center">
            <h1>NHÀ TÀI TRỢ</h1>
        </div>
        <div class="row text-center">
            <h2>NHÀ TÀI TRỢ KIM CƯƠNG</h2>
        </div>
        <div class="row text-center">
            <div class="col-12 text-center">
                <a href="#">
                    <img class="partners" src="/img/InfoContestPage/shanggri-la.png" alt="">
                </a> 
                <a href="#">
                    <img class="partners" src="/img/InfoContestPage/infinity-blockchain-venture.png" alt="">
                </a> 
            </div>
        </div>
        <div class="row text-center">
            <h2>NHÀ TÀI TRỢ VÀNG</h2>
        </div>
        <div class="row text-center">
            <div class="col-12 text-center">
                <a href="#">
                    <img class="partners" src="/img/InfoContestPage/cam-house.png" alt="">
                </a> 
                <a href="#">
                    <img class="partners" src="/img/InfoContestPage/asia-blockchain-reiveiw.png" alt="">
                </a> 
                <a href="#">
                    <img class="partners" src="/img/InfoContestPage/diginet.png" alt="">
                </a> 
            </div>
        </div>
    </div>

    <div class="row text-center">
        <div class="col-6 text-center">
            <h2>NHÀ TÀI TRỢ BẠC</h2>
            <a href="#">
                <img class="partners" src="/img/InfoContestPage/cam-house.png" alt="">
            </a> 
            <a href="#">
                <img class="partners" src="/img/InfoContestPage/asia-blockchain-reiveiw.png" alt="">
            </a> 
        </div>
        <div class="col-6 text-center">
            <h2>NHÀ TÀI TRỢ ĐỒNG</h2>
            <!-- <a href="#">
                <img class="partners" src="/img/InfoContestPage/asia-blockchain-reiveiw.png" alt="">
            </a>  -->
            <a href="#">
                <img class="partners" src="/img/InfoContestPage/shanggri-la.png" alt="">
            </a> 
        </div>
    </div>
    <div class="row text-center">
        <h2>BẢO TRỢ TRUYỀN THÔNG</h2>
    </div>
    <div class="row text-center">
        <div class="col-12 text-center">
            <a href="#">
                <img class="partners" src="/img/InfoContestPage/infinity-blockchain-venture.png" alt="">
            </a> 
            <a href="#">
                <img class="partners" src="/img/InfoContestPage/diginet.png" alt="">
            </a> 
            <a href="#">
                <img class="partners" src="/img/InfoContestPage/shanggri-la.png" alt="">
            </a> 
        </div>
    </div>
</div> 
